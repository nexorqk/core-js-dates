...
function getTime(date) {
  const getTimeString = (time) => time.toString().padStart(2, 0);
  return `${getTimeString(date.getHours())}:${getTimeString(date.getMinutes())}:${getTimeString(date.getSeconds())}`;
}
...
function getNextFriday(date) {
  date.setDate(date.getDate() + ((11 - date.getDay()) % 7) + 1);
  return date;
}
